...
export default function Home() {
  ...
  return <main>
    ...
    {compressionReportOpen && compressionResult && <div className="color-dialog-layer" role="dialog" aria-modal="true" aria-label="一键压缩结果"><div className="color-dialog compression-dialog comparison"><header><div><span className="eyebrow">ONE-CLICK COMPRESSION</span><h3>两种压缩结果</h3></div><button onClick={() => setCompressionReportOpen(false)} aria-label="关闭压缩结果">×</button></header><div className="compression-result-windows"><CompressionResultWindow mode="normal" result={compressionResult.normal} onDownload={() => downloadCompressed("normal")} /><CompressionResultWindow mode="advanced" result={compressionResult.advanced} onDownload={() => downloadCompressed("advanced")} /></div><footer className="compression-footer"><span>普通压缩不改变任何字段；高级压缩采用保真优先的结构优化策略</span><div><button className="secondary" onClick={() => setCompressionReportOpen(false)}>关闭</button></div></footer></div></div>}
